...
def create_notification_queue(result_id, chat_ids):
    """
    ...
    try:
        with conn.cursor() as cur:
            values_sql = ", ".join(["(%s, %s)"] * len(chat_ids))

            params = []
            for chat_id in chat_ids:
                params.extend([result_id, chat_id])
...
def get_notification_queue_count(status=None):
    """
